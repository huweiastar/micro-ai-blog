...
/**
 * 正文图片点击放大（lightbox）。零依赖：事件委托监听 `.prose-custom img` 的点击，
 * 用一个 fixed 遮罩展示放大图，点击遮罩 / Esc 关闭。
 * 全局挂载一次（layout 内），对所有文章/预览生效。reduced-motion 下不做过渡动画。
 */
export function ImageZoom() {
  ...
  useEffect(() => {
    const onClick = (e: MouseEvent) => {
      ...
      if (!target.closest(".prose-custom")) return;
