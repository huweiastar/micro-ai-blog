...
export default async function CategoryDetailPage(props: CategoryPageProps) {
  ...
  return (
    <Container className="py-12">
      ...
      {/* Category Header */}
      <div className="relative mb-10 overflow-hidden rounded-2xl border border-[var(--card-border)]">
        {/* Background */}
        {catConfig.cover ? (
          <>
            <div
            ...
            />
            <div className="bg-[var(--card)]/80 absolute inset-0 backdrop-blur-sm" />
          </>
        ) : (
          <>
            <div className="from-[var(--primary)]/15 via-[var(--accent)]/8 absolute inset-0 bg-gradient-to-br to-transparent" />
            <div className="bg-[var(--card)]/50 absolute inset-0 backdrop-blur-sm" />
          </>
        )}
          ...
          {catConfig.description_long && (
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: await renderMarkdownToHtml(catConfig.description_long),
              }}
            />
          )}
          <span className="bg-[var(--primary)]/15 inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-medium text-[var(--primary)]">
            <svg
